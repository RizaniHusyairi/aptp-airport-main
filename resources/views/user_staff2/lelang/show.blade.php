...
<div class="page-heading">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <x-breadcrumb2 :items="[
                    ...
                    ['label' => 'Lelang/Beauty Contest', 'url' => auth()->user()->is_staff ? route('lelang.staffIndex') : route('lelang.index')],
    ...
    <section class="section">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6 col-12">
                        ...
                        @if($lelang->submission_status == 'Disetujui' && $lelang->reply_document_path)
                        <div class="detail-section">
                            <h6>Surat Balasan:</h6>
                            <a href="{{ $lelang->reply_document_path }}" target="_blank" class="document-link success">
                        ...
                        @if(in_array($lelang->submission_status, ['Ditolak', 'Revisi Diperlukan']) && $lelang->staff_notes)
                        <div class="detail-section">
                            <h6>Catatan dari Staff:</h6>
                            <div class="alert alert-light-{{ $lelang->submission_status == 'Ditolak' ? 'danger' : 'warning' }} mb-0">
                                <p class="mb-0">{{ $lelang->staff_notes }}</p>
                            </div>
                        </div>
                        @endif
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6 col-12">
                        <div class="detail-section">
                        <h6>Tindakan</h6>
                        <form action="{{ route('lelang.updateStatus', $lelang->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            <div class="form-group">
                                <label for="status" class="form-label">Ubah Status</label>
                                <select name="submission_status" id="submission_status" class="form-select">
                                    <option value="Diajukan" @selected($lelang->submission_status == 'Diajukan')>Diajukan</option>
                                    <option value="Disetujui" @selected($lelang->submission_status == 'Disetujui')>Setujui</option>
                                    <option value="Ditolak" @selected($lelang->submission_status == 'Ditolak')>Tolak</option>
                                    <option value="Revisi Diperlukan" @selected($lelang->submission_status == 'Revisi Diperlukan')>Minta Revisi</option>
                                </select>
                            </div>
                            ...
                            <div class="form-group" id="staff-notes-container" style="display: none;">
                                ...
                                <textarea name="staff_notes" id="staff_notes" class="form-control" rows="4" placeholder="Berikan catatan...">{{ $lelang->staff_notes }}</textarea>
                            ...
                            <div class="form-group" id="reply-document-container" style="display: none;">
                                ...
                                <input type="url" name="reply_document_path" id="reply_document_path" class="form-control @error('reply_document_path') is-invalid @enderror" placeholder="https://..." value="{{ old('reply_document_path', $lelang->reply_document_path) }}">
        ...
        <div class="d-flex justify-content-end gap-2">
            <a href="{{ auth()->user()->is_staff ? route('lelang.staffIndex') : route('lelang.index') }}" class="btn btn-secondary">Kembali</a>
